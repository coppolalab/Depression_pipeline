...
ect.design <- model.matrix( ~ 0 + Time.Point, data = pData(lumi.ect) )
...
lumi.nonresponder <- rmcov.ect[,rmcov.ect$responder_to_treatment == "No" & rmcov.ect$Time.Point != "TP4"]
lumi.nonresponder$Time.Point %<>% droplevels
...
expr.collapse <- exprs(lumi.nonresponder) %>% t
...
targets.final.known <- pData(lumi.nonresponder)
#targets.final.known$ %<>% factor(levels = c("CONT", "ECT"))
anova.status <- map_dbl(ME.genes, eigengene.anova, targets.final.known$Time.Point) %>% p.adjust("fdr") %>% signif(3)

cor.status <- map(ME.genes, eigengene.model, targets.final.known$Time.Point)
...
nonresponder.overlaps.list <- map(color.values, module.iterate, modules.out) %>% reduce(cbind) %>% data.frame
...
write.xlsx(nonresponder.overlaps, "./nonresponder.overlaps.xlsx")

nonresponder.pvals <- select(nonresponder.overlaps.list, contains("P.value")) %>% as.matrix
nonresponder.logpvals <- -log10(nonresponder.pvals)
nonresponder.count <- select(nonresponder.overlaps.list, contains("Overlap")) %>% as.matrix
...
heatmap.range <- c(0, 2)
gen.text.heatmap(nonresponder.logpvals, text.matrix.overlaps, color.values, heatmap.labels, "", "nonresponder_genelists", heatmap.range, colorRampPalette(c("red", "yellow", "white"))(n = 300))
