...
timeresponse.TP2 <- rmcov.timeresponse[,rmcov.timeresponse$Time.Point == "TP2"]
...
expr.collapse <- exprs(timeresponse.TP2) %>% t
...
targets.final.known <- pData(timeresponse.TP2)
...
response.TP2.overlaps.list <- map(color.values, module.iterate, modules.out) %>% reduce(cbind) %>% data.frame
response.TP2.overlaps <- data.frame(Gene.List = map_chr(gene.lists, names), Num.Genes = map_int(gene.lists, nrow), response.TP2.overlaps.list)
write.xlsx(response.TP2.overlaps, "./response.TP2.overlaps.xlsx")

response.TP2.pvals <- select(response.TP2.overlaps.list, contains("P.value")) %>% as.matrix
response.TP2.logpvals <- -log10(response.TP2.pvals)
response.TP2.count <- select(response.TP2.overlaps.list, contains("Overlap")) %>% as.matrix
text.matrix.overlaps <- str_c(response.TP2.count, '\n(', signif(response.TP2.pvals, 2), ')')
dim(text.matrix.overlaps) <- dim(response.TP2.count)
heatmap.labels <- str_c(response.TP2.overlaps$Gene.List, " (", response.TP2.overlaps$Num.Genes, ")")
...
heatmap.range <- c(0, 2)
gen.text.heatmap(response.TP2.logpvals, text.matrix.overlaps, color.values, heatmap.labels, "", "response.TP2_genelists", heatmap.range, colorRampPalette(c("red", "yellow", "white"))(n = 300))
