...
gen.enrichrplot <- function(enrichr.df, filename, plot.height = 5, plot.width = 8)
{
    ...
    write.xlsx(baseline1.overlaps, "./baseline1.overlaps.xls")
...
lumi.baseline1 <- readRDS.gz(file = "../baseline/save/baseline1.collapse.rda")
lumi.baseline2 <- readRDS.gz(file = "../baseline/save/baseline2.collapse.rda")

model.baseline1 <- model.matrix( ~ Age + Gender + Ethnicity + RIN, data = pData(lumi.baseline1))[,-1]
baseline1.design <- model.matrix( ~ 0 + Group, data = pData(lumi.baseline1) )
rmcov.baseline1.expr <- removeBatchEffect(exprs(lumi.baseline1), covariates = model.baseline1, design = baseline1.design)
rmcov.baseline1 <- lumi.baseline1 #Make a copy of lumi object
exprs(rmcov.baseline1) <- rmcov.baseline1.expr #Transfer cleaned expression values into new lumi object
...
expr.collapse <- exprs(rmcov.baseline1) %>% t
...
targets.final.known <- pData(rmcov.baseline1)
targets.final.known$Group %<>% factor(levels = c("CONT", "ECT"))
anova.status <- map_dbl(ME.genes, eigengene.anova, targets.final.known$Group) %>% p.adjust("fdr") %>% signif(3)

cor.status <- map(ME.genes, eigengene.model, targets.final.known$Group)
...
baseline1.overlaps.list <- map(color.values, module.iterate, modules.out) %>% reduce(cbind) %>% data.frame
baseline1.overlaps <- data.frame(Gene.List = map_chr(gene.lists, names), Num.Genes = map_int(gene.lists, nrow), baseline1.overlaps.list)
write.xlsx(baseline1.overlaps, "./baseline1.overlaps.xlsx")

baseline1.pvals <- select(baseline1.overlaps.list, contains("P.value")) %>% as.matrix
baseline1.logpvals <- -log10(baseline1.pvals)
baseline1.count <- select(baseline1.overlaps.list, contains("Overlap")) %>% as.matrix
text.matrix.overlaps <- str_c(baseline1.count, '\n(', signif(baseline1.pvals, 2), ')')
dim(text.matrix.overlaps) <- dim(baseline1.count)
heatmap.labels <- str_c(baseline1.overlaps$Gene.List, " (", baseline1.overlaps$Num.Genes, ")")
...
heatmap.range <- c(0, 2)
gen.text.heatmap(baseline1.logpvals, text.matrix.overlaps, color.values, heatmap.labels, "", "baseline1_genelists", heatmap.range, colorRampPalette(c("red", "yellow", "white"))(n = 300))

reduce.overlaps <- select(baseline1.overlaps, Gene.List, Num.Genes, matches("\\.yellow"), matches("\\.green$")) %>% filter(Num.Genes >= 50)
...
gen.text.heatmap(reduce.logpvals, text.matrix.reduce, c("yellow", "green"), reduce.labels, "", "baseline1_genelists_reduce", heatmap.range, colorRampPalette(c("red", "yellow", "white"))(n = 300))
