...
timeresponse.TP3 <- rmcov.timeresponse[,rmcov.timeresponse$Time.Point == "TP3"]
...
expr.collapse <- exprs(timeresponse.TP3) %>% t
...
targets.final.known <- pData(timeresponse.TP3)
...
response.TP3.overlaps.list <- map(color.values, module.iterate, modules.out) %>% reduce(cbind) %>% data.frame
response.TP3.overlaps <- data.frame(Gene.List = map_chr(gene.lists, names), Num.Genes = map_int(gene.lists, nrow), response.TP3.overlaps.list)
write.xlsx(response.TP3.overlaps, "./response.TP3.overlaps.xlsx")

response.TP3.pvals <- select(response.TP3.overlaps.list, contains("P.value")) %>% as.matrix
response.TP3.logpvals <- -log10(response.TP3.pvals)
response.TP3.count <- select(response.TP3.overlaps.list, contains("Overlap")) %>% as.matrix
text.matrix.overlaps <- str_c(response.TP3.count, '\n(', signif(response.TP3.pvals, 2), ')')
dim(text.matrix.overlaps) <- dim(response.TP3.count)
heatmap.labels <- str_c(response.TP3.overlaps$Gene.List, " (", response.TP3.overlaps$Num.Genes, ")")
...
heatmap.range <- c(0, 2)
gen.text.heatmap(response.TP3.logpvals, text.matrix.overlaps, color.values, heatmap.labels, "", "response.TP3_genelists", heatmap.range, colorRampPalette(c("red", "yellow", "white"))(n = 300))
