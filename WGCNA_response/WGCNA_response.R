...
lumi.timeresponse <- ect.collapse[,ect.collapse$Time.Point != "TP4"]

model.timeresponse <- model.matrix( ~ Age + Gender + Ethnicity + RIN, data = pData(lumi.timeresponse))[,-1]
timeresponse.design <- model.matrix( ~ 0 + Time.Response, data = pData(lumi.timeresponse) )
rmcov.timeresponse.expr <- removeBatchEffect(exprs(lumi.timeresponse), covariates = model.timeresponse, design = timeresponse.design)
rmcov.timeresponse <- lumi.timeresponse #Make a copy of lumi object
exprs(rmcov.timeresponse) <- rmcov.timeresponse.expr #Transfer cleaned expression values into new lumi object

timeresponse.TP1 <- rmcov.timeresponse[,rmcov.timeresponse$Time.Point == "TP1"]
...
expr.collapse <- exprs(timeresponse.TP1) %>% t
...
targets.final.known <- pData(timeresponse.TP1)
targets.final.known$responder_to_treatment %<>% factor(levels = c("Yes", "No"))
anova.status <- map_dbl(ME.genes, eigengene.anova, targets.final.known$responder_to_treatment) %>% p.adjust("fdr") %>% signif(3)

cor.status <- map(ME.genes, eigengene.model, targets.final.known$responder_to_treatment)
...
response.TP1.overlaps.list <- map(color.values, module.iterate, modules.out) %>% reduce(cbind) %>% data.frame
response.TP1.overlaps <- data.frame(Gene.List = map_chr(gene.lists, names), Num.Genes = map_int(gene.lists, nrow), response.TP1.overlaps.list)
write.xlsx(response.TP1.overlaps, "./response.TP1.overlaps.xlsx")

response.TP1.pvals <- select(response.TP1.overlaps.list, contains("P.value")) %>% as.matrix
response.TP1.logpvals <- -log10(response.TP1.pvals)
response.TP1.count <- select(response.TP1.overlaps.list, contains("Overlap")) %>% as.matrix
text.matrix.overlaps <- str_c(response.TP1.count, '\n(', signif(response.TP1.pvals, 2), ')')
dim(text.matrix.overlaps) <- dim(response.TP1.count)
heatmap.labels <- str_c(response.TP1.overlaps$Gene.List, " (", response.TP1.overlaps$Num.Genes, ")")
...
heatmap.range <- c(0, 2)
gen.text.heatmap(response.TP1.logpvals, text.matrix.overlaps, color.values, heatmap.labels, "", "response.TP1_genelists", heatmap.range, colorRampPalette(c("red", "yellow", "white"))(n = 300))
