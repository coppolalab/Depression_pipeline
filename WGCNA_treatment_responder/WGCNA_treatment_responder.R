...
lumi.responder <- rmcov.ect[,rmcov.ect$responder_to_treatment == "Yes"]
...
expr.collapse <- exprs(lumi.responder) %>% t
...
targets.final.known <- pData(lumi.responder)
...
responder.overlaps.list <- map(color.values, module.iterate, modules.out) %>% reduce(cbind) %>% data.frame
...
write.xlsx(responder.overlaps, "./responder.overlaps.xlsx")

responder.pvals <- select(responder.overlaps.list, contains("P.value")) %>% as.matrix
responder.logpvals <- -log10(responder.pvals)
responder.count <- select(responder.overlaps.list, contains("Overlap")) %>% as.matrix
...
heatmap.range <- c(0, 2)
gen.text.heatmap(responder.logpvals, text.matrix.overlaps, color.values, heatmap.labels, "", "responder_genelists", heatmap.range, colorRampPalette(c("red", "yellow", "white"))(n = 300))
