...
model.baseline2 <- model.matrix( ~ Age + Gender + Ethnicity + RIN, data = pData(lumi.baseline2))[,-1]
baseline2.design <- model.matrix( ~ 0 + Group, data = pData(lumi.baseline2) )
rmcov.baseline2.expr <- removeBatchEffect(exprs(lumi.baseline2), covariates = model.baseline2, design = baseline2.design)
rmcov.baseline2 <- lumi.baseline2 #Make a copy of lumi object
exprs(rmcov.baseline2) <- rmcov.baseline2.expr #Transfer cleaned expression values into new lumi object
...
expr.collapse <- exprs(rmcov.baseline2) %>% t
...
targets.final.known <- pData(rmcov.baseline2)
...
trap1 <- map(color.names[8:15], enrichr.submit, modules.out, enrichr.terms, FALSE)
...
baseline2.overlaps.list <- map(color.values, module.iterate, modules.out) %>% reduce(cbind) %>% data.frame
baseline2.overlaps <- data.frame(Gene.List = map_chr(gene.lists, names), Num.Genes = map_int(gene.lists, nrow), baseline2.overlaps.list)
write.xlsx(baseline2.overlaps, "./baseline2.overlaps.xlsx")

baseline2.pvals <- select(baseline2.overlaps.list, contains("P.value")) %>% as.matrix
baseline2.logpvals <- -log10(baseline2.pvals)
baseline2.count <- select(baseline2.overlaps.list, contains("Overlap")) %>% as.matrix
text.matrix.overlaps <- str_c(baseline2.count, '\n(', signif(baseline2.pvals, 2), ')')
dim(text.matrix.overlaps) <- dim(baseline2.count)
heatmap.labels <- str_c(baseline2.overlaps$Gene.List, " (", baseline2.overlaps$Num.Genes, ")")

heatmap.range <- c(0, 2)
gen.text.heatmap(baseline2.logpvals, text.matrix.overlaps, color.values, heatmap.labels, "", "baseline2_genelists", heatmap.range, colorRampPalette(c("red", "yellow", "white"))(n = 300))

reduce.overlaps <- select(baseline2.overlaps, Gene.List, Num.Genes, matches("\\.yellow"), matches("\\.red$"), matches("\\.grey60$")) %>% filter(Num.Genes >= 50)
...
gen.text.heatmap(reduce.logpvals, text.matrix.reduce, c("yellow", "red", "grey60"), reduce.labels, "", "baseline2_genelists_reduce", heatmap.range, colorRampPalette(c("red", "yellow", "white"))(n = 300))
